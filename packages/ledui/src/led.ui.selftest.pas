...
begin
  ...
  { A picture inside prose is fitted the same way, but by the page rather
    than by a control: the renderer draws a picture at its natural size and
    cannot scroll a block sideways, so the right-hand side of a wide one was
    simply not there.  Asked of the page the cell renders, because that is
    where the answer is. }
  B := CellBox(Pane, 0);
  if B <> nil then
  begin
    Page := B.ProsePage('![wide](data:image/png;base64,' +
      WidePlotPng + ')', 300);
    Check('a picture wider than the cell is given a width that fits: ' +
      Copy(Page, Pos('<img', Page), 120), Pos('width="300"', Page) > 0);
    Check('and a height in proportion, not squashed flat',
      Pos('height="100"', Page) > 0);
    Page := B.ProsePage('![wide](data:image/png;base64,' +
      WidePlotPng + ')', 2000);
    Check('while one that fits is left at its own size',
      Pos('width=', Page) = 0);
  end;
