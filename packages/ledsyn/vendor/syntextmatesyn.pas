...
{$IF DECLARED(laz_fullversion)}
  {$IF laz_fullversion >= 3000000}
    {$DEFINE LED_SYN_LAZ3_BASE}
  {$ENDIF}
{$ENDIF}
  ...
  private
    ...
    { *** led local patch ***  Lazarus 2.2 keeps the current line in a
      *private* FLineText with no property over it, so capture it in SetLine
      ...
      same string.  See vendor/README.md. }
    FLedLineText: string;
    {$ENDIF}
    function GetParserError: String;

  protected
    {$IFDEF LED_SYN_LAZ3_BASE}
    function GetInstanceLanguageName: string; override;
    {$ELSE}
    function GetInstanceLanguageName: string;
    class function GetLanguageName: string; override;
    property CurrentLineText: string read FLedLineText;
    {$ENDIF}
